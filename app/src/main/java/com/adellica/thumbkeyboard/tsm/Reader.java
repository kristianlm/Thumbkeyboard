...
public class Reader {
            ...
            while ((c = is.read()) >= 0) {
                ...
                switch (c) {
                    case '"':
                        return readString(-1);
                    case ':':
                        return Keypress.fromString(readUntilWS(-1));
                    case '\'':
                        return new Quoted(read());
                    case '{':
                    case '(':
                    case '#':
                    case '?':
                    case '@':
                        throw new ReservedToken(c);
                    case ']':
                        return CLOSE_PAREN;
                    case '[':
                        return readQuotedList();
                    case '0':
                    case '1':
                    case '2':
                    case '3':
            ...
            if (c == '\\') {
                switch (c = is.read()) { // list taken from https://en.wikipedia.org/wiki/Escape_sequences_in_C
                    ...
                    case '?':
                        ...
                        break;
                    case 'x':
                        throw new RuntimeException("\\x parsing TODO");
                    case 'u':
                        throw new RuntimeException("\\u parsing TODO");
                    case 'U':
                        throw new RuntimeException("\\U parsing TODO");
                    default:
    ...
    public static class InvalidEscapeSequence extends TFE {
        public InvalidEscapeSequence(String s) {
            super(s);
        }
    }
